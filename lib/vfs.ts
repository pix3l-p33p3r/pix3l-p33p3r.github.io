...
function projectVisibility(repoUrl?: string): "public" | "42" {
  return repoUrl ? "public" : "42"
}

function projectListing(): string {
  return projects
    .map((project) => {
      const vis = projectVisibility(project.repoUrl)
  ...
  return dir({
    ...
    home: dir({
      guest: dir({
        ...
        "projects.txt": file(`${projectListing()}\n`),
