    ...
    <section id="projects" className="mb-8 pb-5 border-b border-dashed border-[#333] last:border-b-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-5">
        {projects.map((p) => (
          <article
            key={p.slug}
            className="project-card bg-gradient-to-br from-black/60 to-black/80 border border-[#333] p-5 relative overflow-hidden before:content-[''] before:absolute before:top-0 before:left-0 before:w-1 before:h-full before:bg-[#00ffff] before:opacity-70 transition-all duration-300 flex flex-col"
          >
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="flex flex-wrap gap-1.5 mb-2.5 mt-auto">
                {p.tags.map((t) => (
                  <span
                    key={t}
                    className="bg-[rgba(0,255,255,0.1)] border border-[#00ffff] py-1.5 px-3 rounded-full text-sm tracking-wider transition-all duration-300 hover:bg-[rgba(0,255,255,0.2)] hover:-translate-y-0.5"
                  >
                    {t}
                  </span>
                ))}
