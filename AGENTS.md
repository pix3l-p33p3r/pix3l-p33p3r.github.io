...
- Public weekday trading book is file-based JSON under `data/dashboard/` (`book.json`, `trades.json`) loaded by `lib/dashboard.ts` → `/dashboard`. Herald overwrites those files on each ship. Do not invent live brokerage numbers; empty/null means unpublished. Inception points are weekdays only. See `docs/DASHBOARD.md`.
