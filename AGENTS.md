...
- Private site-KPI dashboard is `/admin/analytics` (env password `ADMIN_DASHBOARD_PASSWORD`). Client tracking is Umami (`NEXT_PUBLIC_UMAMI_URL`, `NEXT_PUBLIC_UMAMI_WEBSITE_ID`; optional server `UMAMI_API_TOKEN` for live KPIs). Do not add the dashboard to the public header or sitemap. Setup: `docs/ADMIN_ANALYTICS.md`.
